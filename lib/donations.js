...
export async function getNearbyAvailableDonations(maxMiles) {
    const { data, error } = await supabase.rpc("nearby_available_donations", {
        p_max_miles: maxMiles,
    });

    if (error) throw new Error(error.message);
    return { donations: data }; // already the same shape as getAvailableDonations, plus distance_miles
}